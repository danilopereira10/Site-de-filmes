            ...
            <section id="corpo">
                <section id="resto" style="padding: 0 30px; color: white;">
                    ...
                    <?php
                      ...
                      if ($senha == null)
                      {
                        $sql='UPDATE usuario SET nome="'.$nome.'",sobrenome="'.$sobrenome.'",data_nasc="'.$data.'",sexo="'.$sexo.'", email_user="'.$email.'" where id_user="'.$id_user.'"';
                      }
                      else
                      {
                        $sql='UPDATE usuario SET nome="'.$nome.'",sobrenome="'.$sobrenome.'",data_nasc="'.$data.'",sexo="'.$sexo.'", email_user="'.$email.'", psw_user="'.md5($senha).'" where id_user="'.$id_user.'"';
                      }
                      $rs = mysql_query($sql) or die('Query erro : ' . mysql_error());
                      echo "<br>Dados do usuário editados com sucesso!<hr>";
                      echo "<p>";
